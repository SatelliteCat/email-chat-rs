        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    // Кнопка настроек беседы
                    if ui
                        .add(
                            egui::Button::new(
                                RichText::new("⚙️").font(FontId::proportional(16.0)),
                            )
                            .frame(false),
                        )
                        .on_hover_text("Настройки беседы")
                        .clicked()
                    {
                        sender.send(AppEvent::OpenConversationSettings { conv_id: conv.id });
                    }
