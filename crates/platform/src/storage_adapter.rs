...
fn to_core_group_member(row: storage::models::GroupMemberRow) -> CoreResult<GroupMember> {
    ...
    Ok(GroupMember {
        contact_id: Uuid::parse_str(&row.contact_id)
            .map_err(|e| CoreError::Internal(e.to_string()))?,
        role,
...
impl StoragePort for StorageAdapter {
    ...
    async fn create_group_conversation(
        ...
        let storage_members = members
            .into_iter()
            .map(|(contact_id, role, snapshot)| NewGroupMember {
                contact_id,
                role: match role {
    ...
    async fn find_direct_conversation(
        ...
        match self
            .db
            .conversations()
            .find_direct(account_id, contact_id)
            .await
            .map_err(map_storage_err)?
        {
    ...
    async fn remove_group_member(&self, conv_id: Uuid, contact_id: Uuid) -> CoreResult<()> {
        self.db
            .conversations()
            .remove_member(conv_id, contact_id)
            .await
            .map_err(map_storage_err)
    }
    ...
    async fn get_imap_uids_for_deletion(&self, conv_id: Uuid) -> CoreResult<Vec<ImapUidEntry>> {
        let rows = self
            .db
            .messages()
            .get_imap_uids_for_deletion(conv_id)
            .await
            .map_err(map_storage_err)?;
        Ok(rows
            .into_iter()
    ...
    async fn delete_conversation_messages(&self, conv_id: Uuid) -> CoreResult<()> {
        self.db
            .messages()
            .delete_conversation_messages(conv_id)
            .await
            .map_err(map_storage_err)?;
        Ok(())
    }
